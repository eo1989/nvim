...
local function set_ft_icon_highlight(hl, bg_hl)
  ...
  if bg_color and fg_color then
    ...
    e0.augroup(name, {{events = {"ColorScheme"}, targets = {"*"}, command = str}})
...
function M.file(ctx, minimal)
  ...
  local dir_empty, parent_empty = e0.empty(directory), e0.empty(parent)
  local to_update =
...
local function validate_github_response(data)
  return vim.tbl_islist(data) and not e0.empty(data[1]) and type(data[1]) == "string" and
